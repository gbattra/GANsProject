...
# we're not classifying here, so Y=1 for all examples

d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=Dg, labels=tf.zeros_like(Dg)))
# we're not classifying here, so Y=0 for all examples
...
# set up training for discriminator
...
with tf.variable_scope('generator-optimizer', reuse=tf.AUTO_REUSE):
